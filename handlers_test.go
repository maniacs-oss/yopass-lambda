...
func TestGetSecret(t *testing.T) {
	tt := []struct {
	...
	}{
		{
			...
			output:     "***ENCRYPTED***",
			db:         db1,
			params:     httprouter.Params{httprouter.Param{Key: "key", Value: "foobar"}},
		},
		{
	...
	for _, tc := range tt {
		t.Run(fmt.Sprintf(tc.name), func(t *testing.T) {
			...
			router := httprouter.New()
			router.GET("/secret/:name", func(w http.ResponseWriter, r *http.Request, p httprouter.Params) {
				GetSecret(w, r, p, tc.db)
			})
			rr := httptest.NewRecorder()
			router.ServeHTTP(rr, req)
			json.Unmarshal(rr.Body.Bytes(), &response)
