...
func CreateSecret(w http.ResponseWriter, request *http.Request, p httprouter.Params, db Database) {
...
func GetSecret(w http.ResponseWriter, request *http.Request, p httprouter.Params, db Database) {
	w.Header().Set("Access-Control-Allow-Origin", "*")
	secret, err := db.Get(p.ByName("key"))
	if err != nil {
		fmt.Println(err)
		http.Error(w, `{"message": "Secret not found"}`, http.StatusNotFound)
		return
	}
	err = db.Delete(p.ByName("key"))
	if err != nil {
		fmt.Println(err)
